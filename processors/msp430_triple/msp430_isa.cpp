...
extern bool interrupthappened;
extern int interruptvalue;
...
extern sc_event interruptevent; //extern interruptevent
...
void ac_behavior( instruction ){
...
#ifdef STOPADDRESS
  if (RB.read(PC)>=STOPADDRESS) {
    dbg_printf("%s - PC=%#x ----- %lld\n", (sc_core::sc_get_current_process_b()->get_parent()->name()), (int) RB.read(PC), ac_instr_counter); 	
    dbg_printf("%s - size: %u \n", (sc_core::sc_get_current_process_b()->get_parent()->name()), get_size());
  }
#else
  dbg_printf("%s - PC=%#x ----- %lld\n", (sc_core::sc_get_current_process_b()->get_parent()->name()), (int) RB.read(PC), ac_instr_counter); 	
  dbg_printf("%s - size: %u \n", (sc_core::sc_get_current_process_b()->get_parent()->name()), get_size());
  ...
  //dbg_printf("%s - SR=0x%X ----- \n", (sc_core::sc_get_current_process_b()->get_parent()->name()), RB.read(SR)); 

  // Low power mode	
  while ( (getBit(RB.read(SR), 3)) & (getBit(RB.read(SR), 4)) ) {
    dbg_printf("%s - wait for interrupt - PC=0x%X \n", (sc_core::sc_get_current_process_b()->get_parent()->name()),RB.read(PC));
    wait(interruptevent);  // wait for interrupt
    if (!( (getBit(RB.read(SR), 3)) & (getBit(RB.read(SR), 4)) ) ) {
      dbg_printf("%s - interrupt happened - PC=0x%X \n", (sc_core::sc_get_current_process_b()->get_parent()->name()),RB.read(PC));
      ac_annul(); // cancel current instruction
    }
  }
...
if (interrupthappened){ //dont update in case of interrupt
	RB.write(PC,DM.read(interruptvalue)); //load interrupt vector into PC
	ac_pc = DM.read(interruptvalue);
	interrupthappened=0;
}else{
  if (constant.mode) {
...
void ac_behavior( mov_48idm ){
  ...
  if ((!constant.mode)&& !((sreg != SR && dreg != SR))) {
    if ((dreg != SR) && (ad == 1))
      odst = (uint16_t)(odst + RB.read(dreg));
    else if ((sreg != SR) && (as == 1))
      osrc = (uint16_t)(osrc + RB.read(sreg));
   }
