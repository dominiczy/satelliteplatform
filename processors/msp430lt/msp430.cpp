...
void msp430::behavior() {
  ...
  if( bhv_pc >= dec_cache_size){
    ...
    if( start_up ){
      ...
      if( ins_id == 0 ) {
        cerr << sc_core::sc_get_current_process_b()->get_parent()->basename() << "ArchC Error: Unidentified instruction. " << endl;
...
//!Updating Regs for behavioral simulation.
  ...
  if (ac_stop_flag) {
    ...
    if (instr_in_batch < instr_batch_size) {
      ...
      instr_in_batch = 0;
      wait(250, SC_NS); // 250 ns based on 4 cycles per instruction and 16 mhz clock
    }
  }
