...
#include <string.h>
#include <assert.h>
// DP - not offered by newlib, but abviously not needed anyways
//#include <err.h>
#include <errno.h>
#include <fcntl.h>
#include <unistd.h>

// FB - remove dependence on net libraries
...
#define htonl(x) x
#else
#define htonl(x) ((((x) & 0xff000000u) >> 24) | (((x) & 0x00ff0000u) >>  8) |  \
...
main(int argc, char **argv)
{
	...
	unsigned long mask=0xffffffff;
        // FB - avoid use of floating-point
	unsigned int time;
...
// 	}
	...
	while (fgets(line, 128, fp)) {
		 ...
		 */
                // FB - avoid use of floating-point
		sscanf(line, "%d %d", &time, (unsigned int *)&addr);
		...
		{
			printf("%10d %08x: ", (unsigned int)time, addr.s_addr);
			printf("Found.\n");
		}
		else
		{
