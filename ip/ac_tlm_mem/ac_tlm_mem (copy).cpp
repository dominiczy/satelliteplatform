...
ac_tlm_mem::ac_tlm_mem( sc_module_name module_name , int k ) :
  ...
  target_export("iport"),
  target_export2("iport2"),  //2nd port
  target_export3("iport3")  //3rd port
{
    /// Binds target_export to the memory
    target_export( *this );
    target_export2( *this ); //bind 2nd port
    target_export3( *this ); //bind 3rd port
...
ac_tlm_rsp_status ac_tlm_mem::readm( const uint32_t &a , uint32_t &d )
{
  *((uint32_t *) &d) = *((uint32_t *) &memory[a]);
	...
	if (a==UCB0RXBUF_){ // even mem read rxbuf
		cout << "MEM: rxbuf read notify "<< a <<  endl;		
