...
struct ac_tlm_mem :
  sc_module,
  ac_tlm_transport_if // Using ArchC TLM protocol
{
public:
  
  /// Exposed port with ArchC interface
  sc_export< ac_tlm_transport_if >* target_export[N_TARGETS];
  ...
  */
  ac_tlm_rsp transport( const ac_tlm_req &request ) {
    ...
    case READ :     // Packet is a READ one
      ...
      cout << "MEM: "<< name() << " ----- Transport READ at 0x" << hex << request.addr << " value ";  //if addr of peripheral module send interrupt about read/write
      #endif
    ...
    case WRITE:     // Packet is a WRITE
      #ifdef DEBUG
    cout << "MEM: "<< name() << " ----- Transport WRITE at 0x" << hex << request.addr << " value ";
   ...
   *
   */
  ac_tlm_mem( sc_module_name module_name , int k = 128*1024 );
